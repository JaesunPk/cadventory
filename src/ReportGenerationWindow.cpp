...
void ReportGenerationWindow::coverPage() {
  ...
  */
  ...
  int title_width = A4_MAXWIDTH_LS - 2 * mx;          // 2908
  int title_height = A4_MAXHEIGHT_LS - 2 * my - 300;  // 1680
  QRect title_rect(
      mx, my, A4_MAXWIDTH_LS - 2 * mx,
      A4_MAXHEIGHT_LS - 2 * my -
          300);  // 300 is arbitrary, need to decrease the rectangle to allocate
                 // space for subtitle, classification
  ...
  // label
  QRect label_rect(mx, my + title_height + 100, title_width, 100);
  label = ui->label_textEdit->toPlainText().toStdString();

  // painter->drawRect(label_rect);
  painter->drawText(label_rect, Qt::AlignCenter, QString::fromStdString(label));
